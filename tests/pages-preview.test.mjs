...
test('GitHub Pages preview has branded build sources and deployment workflow',()=>{
  for(const path of ['scripts/build-preview.mjs','preview/index.html','preview/styles.css','preview/brand-refresh.css','preview/app.js','preview/image-assets.json','preview/image-assets.js','preview/site.webmanifest','preview/robots.txt','preview/sitemap.xml','.github/workflows/pages.yml']){
...
test('Pages preview build emits all ten products, botanical branding, disclosures, and coming-soon checkout',()=>{
  ...
  for(const path of ['preview/dist/index.html','preview/dist/styles.css','preview/dist/brand-refresh.css','preview/dist/app.js','preview/dist/image-assets.js','preview/dist/site.webmanifest','preview/dist/robots.txt','preview/dist/sitemap.xml','preview/dist/products.json','preview/dist/.nojekyll','preview/dist/photo-credits.html']){
...
test('representative image manifest covers all ten sample plants and uses stable Shopify CDN assets',()=>{
  ...
  assert.match(manifest.appleTouchIcon,/^https:\/\/cdn\.shopify\.com\//);
